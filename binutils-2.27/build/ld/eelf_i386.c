...
static void gldelf_i386_after_parse (void);
...
static lang_output_section_statement_type *gldelf_i386_place_orphan
...
static void
gldelf_i386_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf_i386_append_to_separated_string (&depaudit, cp);
...
OUTPUT_ARCH(i386)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/Users/andrewowens/.local/binutils/i386-unknown-linux-gnu/lib32\"); SEARCH_DIR(\"/Users/andrewowens/.local/binutils/i386-unknown-linux-gnu/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf_i386_handle_option (int optc)
{
  switch (optc)
	...
	einfo (_("%P%F: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldelf_i386_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf_i386_append_to_separated_string (&depaudit, optarg);
	break;
