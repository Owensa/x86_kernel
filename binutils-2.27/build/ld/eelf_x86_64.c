...
static void gldelf_x86_64_after_parse (void);
...
static void
gldelf_x86_64_after_parse (void)
{
...
gldelf_x86_64_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf_x86_64_try_needed (&nn, TRUE))
	    continue;
	}
...
static void
gldelf_x86_64_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf_x86_64_append_to_separated_string (&depaudit, cp);
...
static lang_output_section_statement_type *
gldelf_x86_64_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldelf_x86_64_after_allocation (void)
{
  ...
  if (need_layout < 0)
    ...
    gldelf_x86_64_map_segments (need_layout);
}
...
OUTPUT_ARCH(i386:x86-64)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/Users/andrewowens/.local/binutils/i386-unknown-linux-gnu/lib64\"); SEARCH_DIR(\"/Users/andrewowens/.local/binutils/i386-unknown-linux-gnu/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf_x86_64_handle_option (int optc)
{
  switch (optc)
	...
	einfo (_("%P%F: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldelf_x86_64_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf_x86_64_append_to_separated_string (&depaudit, optarg);
	break;
