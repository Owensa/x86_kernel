...
static void gldelf32_x86_64_after_parse (void);
...
static lang_output_section_statement_type *gldelf32_x86_64_place_orphan
  (asection *, const char *, int);

static void
gldelf32_x86_64_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldelf32_x86_64_after_parse (void)
{
...
static bfd_boolean
gldelf32_x86_64_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf32_x86_64_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32_x86_64_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32_x86_64_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32_x86_64_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32_x86_64_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32_x86_64_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf32_x86_64_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf32_x86_64_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf32_x86_64_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32_x86_64_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32_x86_64_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32_x86_64_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32_x86_64_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf32_x86_64_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32_x86_64_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32_x86_64_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32_x86_64_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32_x86_64_find_exp_assignment (exp->binary.lhs);
      gldelf32_x86_64_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32_x86_64_find_exp_assignment (exp->trinary.cond);
      gldelf32_x86_64_find_exp_assignment (exp->trinary.lhs);
      gldelf32_x86_64_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32_x86_64_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32_x86_64_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32_x86_64_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldelf32_x86_64_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
  ...
  if (is_elf_hash_table (link_info.hash))
    {
	 ...
	 referred to by dynamic objects.  */
      lang_for_each_statement (gldelf32_x86_64_find_statement_assignment);
    }
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32_x86_64_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldelf32_x86_64_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf32_x86_64_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldelf32_x86_64_after_allocation (void)
{
  ...
  if (need_layout < 0)
    ...
    gldelf32_x86_64_map_segments (need_layout);
}
...
OUTPUT_ARCH(i386:x64-32)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/Users/andrewowens/.local/binutils/i386-unknown-linux-gnu/libx32\"); SEARCH_DIR(\"/Users/andrewowens/.local/binutils/i386-unknown-linux-gnu/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf32_x86_64_handle_option (int optc)
{
  switch (optc)
	...
	einfo (_("%P%F: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldelf32_x86_64_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf32_x86_64_append_to_separated_string (&depaudit, optarg);
	break;
